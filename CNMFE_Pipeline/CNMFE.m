...
%neuron.viewNeurons([], neuron.C_raw); % No need, ComponentManager will be used to select neurons
tic;
cnmfe_update_BG;
fprintf('Time cost in estimating the background:        %.2f seconds\n', toc);
%update spatial & temporal components
...
for m=1:2
    ...
    %spatial
    if m == 2                       %% REMOVE AFTER DEBUGGING %%
        disp('Coren breakpoint');   %% REMOVE AFTER DEBUGGING %%
